...
class ChangeDoneTradesCommand(newDoneTrades: ArrayBuffer[TradeDoneCalculations]) extends GameCommand {
  override def execute(state: GameState): GameState = {
    new DefaultGameState {
      ...
      override def doneTrades: ArrayBuffer[TradeDoneCalculations] = newDoneTrades // Use newDoneTrades here

...
class ChangeDatabaseConnectionStringCommand(newDatabaseConnectionString: String) extends GameCommand {
  override def execute(state: GameState): GameState = {
    new DefaultGameState {
      ...
      override def databaseConnectionString: String = newDatabaseConnectionString

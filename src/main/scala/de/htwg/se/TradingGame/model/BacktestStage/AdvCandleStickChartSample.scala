...
def addCandlesFromTo(chart: CandleStickChart, interval: String, symbol: String, startDate: LocalDateTime, endDate: LocalDateTime): Unit = {
  // Fetch the candle data in a separate thread
  val futureCandles = Future {
    getCandleDataFromTo(interval, symbol, startDate, endDate)
  }
...
def adjustCandlestoleft(chart: CandleStickChart, interval: String, symbol: String, startDate: LocalDateTime, size: Int): Unit = {
    
        // Add candles to the left
    addCandlesToLeft(chart,interval, symbol, startDate, size)
    println("added candles to left")
    if (!getHighestXTimeIsInBounds(chart)) {
        // Delete candles from the right
        deleteCandlesFromRight(chart, size)
        println("deleted candles from right")
    }
...
//     if(isBacktestDateInBounds(chart)){
//         // val formatter = DateTimeFormatter.ofPattern("yyyy.MM.dd,HH:mm")
//         // val enddate = LocalDateTime.parse(TradeData.backtestDate, formatter)
//         // addCandlesFromTo(chart, interval, symbol, startDate, enddate )
//         // println("added candles from to" + startDate + " " + enddate)
//     }else{
//         // Add candles to the right
//         addCandlesToRight(chart, interval, symbol, startDate, size)
//         println("added candles to right")
//     }

//   // Delete candles from the left
//   if (!getLowestXTimeisinBounds(chart)) {
//     deleteCandlesFromLeft(chart, size)
//     println("deleted candles from left")
//   }
...
def getLowestXTimeisinBounds(chart: CandleStickChart): Boolean = {
    ...
    if(getLowestXTime(chart).isDefined && getLowestXTime(chart).get > axisLowerBound - (1000*distancecandles)) {
        return true
    }else{
        return false
...
def getDateInBounds(chart: CandleStickChart, epochSeconds: Long): Boolean = {
  val xAxis = chart.getXAxis
  val getLowerBoundMethod = xAxis.getClass.getMethod("getLowerBound")
  val getUpperBoundMethod = xAxis.getClass.getMethod("getUpperBound")
  val axisLowerBound = getLowerBoundMethod.invoke(xAxis).asInstanceOf[Double]
  ...
  if(epochSeconds > axisLowerBound - (1000*distancecandles) && epochSeconds < axisUpperBound + (1000*distancecandles)) {
    return true
  } else {
    return false
  }
...
// def isBacktestDateInBounds(chart: CandleStickChart): Boolean = {
//   // Convert the backtestDate from a string to a LocalDateTime object
//   // val formatter = DateTimeFormatter.ofPattern("yyyy.MM.dd,HH:mm")
//   // val dateTime = LocalDateTime.parse(TradeData.backtestDate, formatter)

//   // Convert the LocalDateTime object to epoch seconds
//   val epochSeconds = dateTime.atZone(ZoneId.systemDefault()).toEpochSecond()

//   // Get the lower and upper bounds of the X axis
//   val xAxis = chart.getXAxis
//   val getUpperBoundMethod = xAxis.getClass.getMethod("getUpperBound")
//   val axisUpperBound = getUpperBoundMethod.invoke(xAxis).asInstanceOf[Double]

//   // Check if the epoch seconds is within the bounds of the X axis
//   epochSeconds <= axisUpperBound + (1000*distancecandles)
// }

// def isEndDateInBounds(chart: CandleStickChart): Boolean = {
//   // Convert the endDate from a string to a LocalDateTime object
//   // val formatter = DateTimeFormatter.ofPattern("yyyy.MM.dd,HH:mm")
//   // val dateTime = LocalDateTime.parse(TradeData.endDate, formatter)
...
def getHighestVisibleXTime(chart: CandleStickChart): Option[Double] = {
  // Flatten the data from all series into a single collection
  val allData = chart.data.flatMap(_.getData)

  // Filter the data points that are visible
  val visibleData = allData.filter(data => data.getNode.asInstanceOf[Candle].isVisible)

  // Find the data point with the highest x value
  val maxData = visibleData.maxByOption(_.getXValue.doubleValue())
